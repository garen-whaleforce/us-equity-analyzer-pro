...
app.post('/api/analyze', async (req,res)=>{
  ...
  try{
    ...
    // 3) Price Target 多來源備援
    let ptAgg;
    try{
      ptAgg = await getAggregatedPriceTarget(ticker, FH_KEY, AV_KEY);
    }catch(e){
      ptAgg = { error: e.message };
    }
    ...
    res.json({
      ...
      fetched:{
        ...
        finnhub_summary:{
          recommendation: finnhub.recommendation?.[0] || finnhub.recommendation,
          quote: finnhub.quote,
          price_target: ptAgg
        }
      },
