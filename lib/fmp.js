...
// ═══════════════════════════════════════════════════════════════════════════
// Rate Limiter - 限制 FMP API 每秒請求數，避免 429 錯誤
// ═══════════════════════════════════════════════════════════════════════════
const FMP_MAX_RPS = 8; // 每秒最多 8 個請求 (Premium 通常允許 10-30)
const FMP_INTERVAL_MS = 1000 / FMP_MAX_RPS; // 每個請求間隔 125ms
...
async function processQueue() {
  ...
  while (requestQueue.length > 0) {
    ...
    if (elapsed < FMP_INTERVAL_MS) {
      await new Promise(resolve => setTimeout(resolve, FMP_INTERVAL_MS - elapsed));
    }
    ...
    try {
      const response = await axios.get(config.url, config.options);
      resolve(response);
    } catch (err) {
      reject(err);
    }
...
function throttledGet(url, options = {}) {
  return new Promise((resolve, reject) => {
    requestQueue.push({ resolve, reject, config: { url, options } });
    processQueue();
  });
}
// ═══════════════════════════════════════════════════════════════════════════
...
export async function getFmpQuote(symbol, key){
  ...
  const { data } = await throttledGet(url,{ params, timeout:15000 });
...
export async function getFmpBatchQuote(symbols, key){
  ...
  for(let idx=0; idx<unique.length; idx+=BATCH_QUOTE_CHUNK){
    ...
    const { data } = await throttledGet(BATCH_QUOTE_ENDPOINT, { params, timeout:15000 });
...
export async function getFmpHistorical(symbol, date, key){
  ...
  const { data } = await throttledGet(HISTORICAL_EOD_FULL,{ params, timeout:20000 });
...
export async function getFmpAnalystEstimates({ symbol, period='annual', page=0, limit=8 }, key){
  ...
  const { data } = await throttledGet(url,{ params, timeout:20000 });
...
export async function getFmpProfile(symbol, key){
  ensureKey(key);
  const url = `${FMP_BASE_STABLE}/profile`;
  const params = { symbol, apikey:key };
  const { data } = await throttledGet(url,{ params, timeout:15000 });
  if(Array.isArray(data) && data.length) return data[0];
  if(data && data.symbol) return data;
  throw new Error('[FMP] profile empty');
}
...
export async function getFmpTechnicalIndicator({ symbol, interval='daily', type='sma', period=20, series_type='close', fast=12, slow=26, signal=9 }, key){
  ...
  const { data } = await throttledGet(url,{ params, timeout:DEFAULT_TIMEOUT });
  return Array.isArray(data) ? data : [];
}
...
export async function getFmpStockEtfs(symbol, key){
  ensureKey(key);
  const params = { symbol, apikey:key };
  const { data } = await throttledGet(`${FMP_BASE_STABLE}/etf/asset-exposure`,{ params, timeout:DEFAULT_TIMEOUT });
  return Array.isArray(data) ? data : [];
}
...
export async function getFmpInsiderTrading({ symbol, limit=20, page=0, from, to }, key){
  ...
  const { data } = await throttledGet(`${FMP_BASE_STABLE}/insider-trading/search`,{ params, timeout:DEFAULT_TIMEOUT });
  return Array.isArray(data) ? data : [];
}
...
export async function getFmpInsiderStats({ symbol, period='monthly', limit=4 }, key){
  ...
  const { data } = await throttledGet(`${FMP_BASE_STABLE}/insider-trading/statistics`,{ params, timeout:DEFAULT_TIMEOUT });
...
export async function getFmpAnalystActions({ symbol, limit=20, page=0 }, key){
  ensureKey(key);
  const params = { symbol, apikey:key };
  const { data } = await throttledGet(`${FMP_BASE_STABLE}/grades`,{ params, timeout:DEFAULT_TIMEOUT });
  const rows = Array.isArray(data) ? data : [];
  if(!rows.length || !Number.isFinite(limit)) return rows;
...
export async function getFmpEconomicCalendar({ from, to }, key){
  ...
  const { data } = await throttledGet(`${FMP_BASE_STABLE}/economic-calendar`,{ params, timeout:DEFAULT_TIMEOUT });
  return Array.isArray(data) ? data : [];
}
...
export async function getFmpTreasuryCurve({ from, to, maturity }, key){
  ensureKey(key);
  const params = { from, to, apikey:key };
  const { data } = await throttledGet(`${FMP_BASE_STABLE}/treasury-rates`,{ params, timeout:DEFAULT_TIMEOUT });
...
export async function getFmpMarketRiskPremium({ country='United States' } = {}, key){
  ...
  const { data } = await throttledGet(`${FMP_BASE_STABLE}/market-risk-premium`,{ params, timeout:DEFAULT_TIMEOUT });
...
export async function getFmpInstitutionalHolders({ symbol, year, quarter, key, page=0, limit=100 }){
  ...
  const [holdersRes, summaryRes] = await Promise.allSettled([
    throttledGet(`${FMP_BASE_STABLE}/institutional-ownership/extract-analytics/holder`,{ params, timeout:30000 }),
    throttledGet(`${FMP_BASE_STABLE}/institutional-ownership/symbol-positions-summary`,{ params: summaryParams, timeout:15000 })
  ]);
...
export async function getFmpEarningsCallTranscript({ symbol, quarter, year, key }){
  ...
  const { data } = await throttledGet(`${FMP_BASE_STABLE}/earning-call-transcript`,{ params, timeout:30000 });
  return Array.isArray(data) ? data : [];
}
