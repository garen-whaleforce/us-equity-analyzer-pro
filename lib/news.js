...
function articleMatchesTicker(article, ticker){
  ...
  if(symbolSet.has(target)) return true;
  const haystack = `${article.title || ''} ${article.summary || ''}`.toUpperCase();
  return target.length >= 2 && haystack.includes(target);
}

function filterRelevantArticles(articles, ticker, keywords=[]){
  ...
  for(const article of articles){
    const haystack = `${article.title || ''} ${article.summary || ''}`.toUpperCase();
    if(articleMatchesTicker(article, ticker)){
      primary.push(article);
    }else if(keywordPool.length && keywordPool.some(kw=>haystack.includes(kw))){
...
export async function buildNewsBundle({ ticker, baselineDate, openKey, model, useLlm=true, articleLimit=5, finnhubKey, fmpKey }){
  ...
  try{
    ...
    articles = filterRelevantArticles(articles, ticker, keywords);
