...
async function callOpenAI(openKey, model, messages, cachePrefix, ttl){
...
export async function getNewsKeywords(ticker, openKey, model){
  ...
  try{
    const text = await callOpenAI(openKey, model, prompt, 'news_kw_resp', KEYWORD_TTL);
...
export async function analyzeNewsSentiment({ ticker, baselineDate, articles, openKey, model }){
  ...
  try{
    const text = await callOpenAI(openKey, model, messages, 'news_sentiment', NEWS_CACHE_TTL);
