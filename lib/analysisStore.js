...
const SCHEMA_VERSION = 'analysis_v1';
...
export function getCachedAnalysis({ ticker, baselineDate, ttlMs }){
  if(!ticker || !baselineDate || !ttlMs) return null;
  try{
    const row = getStmt.get(ticker, baselineDate, SCHEMA_VERSION);
    if(!row) return null;
...
export function saveAnalysisResult({ ticker, baselineDate, isHistorical, result }){
