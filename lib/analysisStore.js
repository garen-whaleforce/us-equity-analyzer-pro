...
const BASE_SCHEMA_VERSION = 'analysis_v3';
...
const deleteStmt = db.prepare(`DELETE FROM analyses WHERE ticker=? AND baseline_date=? AND schema_version=?`);
const selectRawStmt = db.prepare(`SELECT result_json FROM analyses WHERE ticker=? AND baseline_date=? AND schema_version=?`);
...
export function getCachedAnalysis({ ticker, baselineDate, ttlMs, model }){
  ...
  try{
    ...
    if(parsed?.analysis?.action?.rating && parsed?.analysis?.profile?.score!=null){
      return parsed;
    }
    return null;
  }catch(err){
...
export function getStoredResult({ ticker, baselineDate, model }){
  if(!ticker || !baselineDate) return null;
  try{
    const row = selectRawStmt.get(ticker, baselineDate, versionKey(model));
    return row ? JSON.parse(row.result_json) : null;
  }catch(err){
    console.warn('[analysisStore] getStoredResult failed', err.message);
    return null;
  }
}

export function deleteAnalysis({ ticker, baselineDate, model }){
  if(!ticker || !baselineDate) return;
  try{
    deleteStmt.run(ticker, baselineDate, versionKey(model));
  }catch(err){
    console.warn('[analysisStore] delete failed', err.message);
  }
}
